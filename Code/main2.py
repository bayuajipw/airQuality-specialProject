...
while True:
    ...
    if perc1['CO'] is not None and perc1['SMOKE'] is not None and Kelembapan is not None and Suhu is not None:
        print('Suhu: {0:0.1f} C Kelembapan: {1:0.1f}%'.format(Suhu, Kelembapan))
        sys.stdout.write('CO: %.2f ppm, SMOKE: %.2f ppm\n' % (perc1['CO'], perc1['SMOKE']))
        ...
        if Suhu < 18 or Suhu > 30:
            ...
            pass
        # Kelembapan Ruangan
        if Kelembapan < 40 or Kelembapan > 60:
            GPIO.output(Buzzer, GPIO.HIGH)
            sleep(D)
            GPIO.output(Buzzer, GPIO.LOW)
            client.publish('Kel', json.dumps(N), 1)  # publish
        elif Kelembapan >= 40 or Kelembapan <= 60:
            GPIO.output(Buzzer, GPIO.LOW)
        ...
        if flame_state == GPIO.HIGH:
            ...
            flame_status = "Ada Api"
        else:
            client.publish('Flame', json.dumps(P), 1)  # publish
            flame_status = "Tidak ada Api"
        ...
        # Fan Control
        if Suhu > 20 or perc1['CO'] > 25 or perc1['SMOKE'] > 25:
